...
export default function PokemonTCG() {
  ...
  const analyzeItem = async () => {
    ...
    try {
      ...
      const mimeType = scanPreview.substring(5, scanPreview.indexOf(';'));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <Card className="border-none shadow-sm bg-card">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Total de Artículos</p>
              <h3 className="text-xl font-black mt-1">{stats.totalQty} unidades</h3>
              <p className="text-[10px] text-muted-foreground mt-0.5">{stats.uniqueCount} referencias únicas</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-sm bg-card">
          <CardContent className="p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Valor Colección (USD)</p>
              <h3 className="text-xl font-black mt-1 text-yellow-600">
        ...
        <Card className="border-none shadow-sm bg-card bg-emerald-50/20 dark:bg-emerald-950/10">
          <CardContent className="p-4 flex items-center gap-4">
            <div className="p-3 bg-emerald-100 dark:bg-emerald-950/50 rounded-xl text-emerald-600 dark:text-emerald-400">
              <Coins className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs font-bold text-emerald-600/80 dark:text-emerald-400 uppercase tracking-wider">Valor Equiv. (COP)</p>
              <h3 className="text-xl font-black mt-1 text-emerald-600 dark:text-emerald-400">{fmt(stats.totalCop)}</h3>
              <p className="text-[10px] text-muted-foreground mt-0.5">Tasa USDT: {fmt(usdtRate)}</p>
            </div>
          </CardContent>
        </Card>

        <Card className="border-none shadow-sm bg-card bg-blue-50/20 dark:bg-blue-950/10">
          <CardContent className="p-4 flex items-center gap-4">
            <div className={cn(
              "p-3 rounded-xl",
            ...
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Inversión y ROI</p>
              {stats.totalInvestedUsd > 0 ? (
                <>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className={cn(
                      ...
                      stats.roiPct >= 0 ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-extrabold" : "bg-rose-500/10 text-rose-600 dark:text-rose-400 font-extrabold"
                    )}>
                      {stats.roiPct >= 0 ? '+' : ''}{stats.roiPct.toFixed(1)}%
                    </span>
                    <span className="text-[9px] text-muted-foreground font-mono">
                      ({stats.totalGainLossUsd >= 0 ? '+' : ''}${stats.totalGainLossUsd.toFixed(1)} USD)
                    </span>
                  </div>
                </>
              ) : (
                <>
                  <h3 className="text-xl font-black mt-1 text-muted-foreground">$0.00 USD</h3>
                  <p className="text-[10px] text-muted-foreground mt-0.5">Opcional en agregar/editar</p>
                </>
              )}
            </div>
          </CardContent>
        ...
        <Card className="border-none shadow-sm bg-card">
          <CardContent className="p-4 flex items-center gap-4">
            <div className="p-3 bg-slate-100 dark:bg-slate-900 rounded-xl text-slate-600">
              <ShoppingBag className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Distribución</p>
              <h3 className="text-lg font-black mt-1">{stats.cardsQty} cartas / {stats.sealedQty} sellados</h3>
      ...
      <Dialog open={isScanOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-4xl bg-card border-none shadow-2xl p-5 md:p-6 overflow-y-auto max-h-[95vh] md:max-h-[90vh]">
          <style>{`
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 pt-3">
            ...
            <div className="lg:col-span-7 space-y-4 flex flex-col justify-between">
              ...
              {/* Tabs */}
              <div className="flex border-b border-border/80 bg-muted/20 p-1 rounded-lg">
                <button
                  type="button"
                  onClick={() => {
                    setActiveTab('camera');
                  ...
                  className={cn(
                    "flex-1 py-1.5 px-3 text-[10px] sm:text-xs font-black uppercase tracking-wider text-center rounded-md transition-all flex items-center justify-center gap-1.5",
                    activeTab === 'camera' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
                  )}
                >
                  <Camera className="w-3.5 h-3.5 text-yellow-500" /> Cámara en Vivo
                </button>
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  <Upload className="w-3.5 h-3.5 text-blue-500" /> Subir Archivo
                </button>
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  <Search className="w-3.5 h-3.5 text-emerald-500" /> Búsqueda por Texto
                </button>
              </div>
